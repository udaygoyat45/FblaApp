...
def account():
    ...
    return render_template("account.html", title="Your Account", form=form, user=user, head=False, forms=True, image="../static/img/plane.jpg")
...
def final():
    ...
    if form.validate_on_submit():
        curr = UserFlight(date=datetime.datetime.strptime(form.date.data, '%m/%d/%Y %S%M%H'),
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template("reset_request.html", name="Reset Password", head=False, form=form, forms=True, title="Reset Password", image="../static/img/plane.jpg")
